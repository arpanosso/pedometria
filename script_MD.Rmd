...
**Exercício 10**. Ordene os registro por ordem crescente de teor de ARGILA
```{r eval=FALSE}
dados %>% 
  arrange(ARGILA)
```

**Exercício 11**. Ordene os registro por ordem decrescente de "H_Al".
```{r eval=FALSE}
dados %>% 
  arrange(desc(H_Al))
```
...
**Exercício 14**. Calcule as estatísticas descrivas para alguns atributos do solo de sua preferência. 
